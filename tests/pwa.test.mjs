...
test("service worker never caches online API or mutation requests", () => {
  ...
  assert.match(worker, /2026-07-30-pwa-brand-v2/);
  assert.match(worker, /icon-v2-512\.png/);
  assert.match(worker, /installed-splash-v2\.webp/);
...
test("updates are offered only on safe entry screens", () => {
  ...
  const installedSplash = read(
    "app/components/InstalledPwaSplash.tsx",
  );
  ...
  assert.match(layout, /<InstalledPwaSplash \/>/);
  assert.match(layout, /suppressHydrationWarning/);
  assert.match(layout, /<PwaLifecycle \/>/);
  assert.match(installedSplash, /dalmuti-installed-splash-v2/);
  assert.match(installedSplash, /display-mode: standalone/);
  assert.match(installedSplash, /userAgentData/);
  assert.match(installedSplash, /source"\) ===\s*"android-twa"/);
  assert.match(installedSplash, /INSTALLED_SPLASH_CLEANUP_MS = 1_400/);
  assert.match(
    globalStyles,
    /display-mode: standalone\) and \(hover: none\) and \(pointer: coarse\)/,
  );
  assert.match(globalStyles, /installed-splash-v2\.webp/);
  assert.match(globalStyles, /installedPwaSplashLayer 1\.34s/);
});
...
test("installed Android app uses one branded splash and system-aware rotation", () => {
  ...
  assert.match(
    nativeLauncher,
    /R\.drawable\.dalmuti_splash_glow_v4/,
  );
  assert.match(nativeLauncher, /R\.drawable\.dalmuti_splash_v4/);
  ...
  for (const density of ["mdpi", "hdpi", "xhdpi", "xxhdpi", "xxxhdpi"]) {
    for (const [folder, file] of [
      ...
      ["drawable", "dalmuti_splash_glow_v4.png"],
      ["mipmap", "dalmuti_app_icon_v3.png"],
      ["mipmap", "dalmuti_app_icon_maskable_v3.png"],
    ]) {
      assert.equal(
        fs.existsSync(
          path.join(
            root,
            `android-twa/custom/res/${folder}-${density}/${file}`,
          ),
        ),
        true,
        `${folder}-${density}/${file} is missing`,
      );
    }
  }
  assert.equal(
  ...
  assert.match(assetBuilder, /dalmuti-splash-v4\.png/);
...
<manifest xmlns:android="http://schemas.android.com/apk/res/android">
  <application android:icon="@mipmap/ic_launcher"
    ...
    <activity android:name="LauncherActivity"
      ...
      assert.match(
        customizedManifest,
        /android:resource="@drawable\/dalmuti_splash_v4"/,
      );
      assert.match(
        customizedManifest,
      ...
      const copiedSplash =
        "app/src/main/res/drawable-xxxhdpi/dalmuti_splash_v4.png";
      assert.equal(
        sha256(path.relative(root, path.join(fixtureRoot, copiedSplash))),
        sha256(
          "android-twa/custom/res/drawable-xxxhdpi/dalmuti_splash_v4.png",
        ),
      );
...
test("required offline and install assets exist", () => {
  for (const file of [
    ...
    "public/pwa/installed-splash-v2.webp",
  ]) {
