...
| Opening scale by player count | installed quick rank draw / confirmation / reveal / tax / revolution | installed online equivalents | Portrait rank cards keep one physical `46–54px` width for 4–10 players; only row count changes. Short landscape uses `42px` cards in one row. Every other pre-game card and overlay consumes the same shared installed-app dimensions in both modes | Shared / fixed |
...
| Desktop own-hand centering | quick full-width `.hand-wrap` | online `handScrollerRef` + full-width `.handScroller` | Status panel never shifts the hand axis; initial deal and every card-count mutation recenter the online scroll viewport | Shared / fixed |
